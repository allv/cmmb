...
var $2 = function(id){
    ...
    function AddRow() {
        ...
        row.insertCell(row.cells.length).innerHTML = "<input type='text' size='21' name='sstarttime' value='' class='Wdate' onClick=\"WdatePicker({dateFmt:'HH:mm:ss',readOnly:true})\" />";
        row.insertCell(row.cells.length).innerHTML = "<input type='text' size='21' name='sendtime' value='' class='Wdate' onClick=\"WdatePicker({dateFmt:'HH:mm:ss',readOnly:true})\" />";
        row.insertCell(row.cells.length).innerHTML = "<input type='text' name='sattend' value='${sattend}' size='28'/>";
        row.insertCell(row.cells.length).innerHTML = "<textarea name='sinfo' cols='38' onBlur=\"javascript:this.style.width='204px';this.style.height='21px';\" onFocus=\"javascript:this.style.width='210px';this.style.height='90px';\" rows='1'>${sinfo}</textarea>";
        row.insertCell(row.cells.length).innerHTML = "<input type='text' name='swrite' value='${swrite}' size='14'/>";
 ...
 function choosePro(){
    ...
    function Save() {
        ...
        for (var i = 0; i < tbody.rows.length; i++) {
            ...
            var item = sstarttime + "^" + sendtime + "^" + sattend + "^" + sinfo + "^" + swrite + "^" + sbelongpro;
            detail.push(item);
        }
        ...
        else
        {
             $.ajax({
					type : "POST",
					url : basePath + "/sensorysaveSenchology.do?detail=" + detailstr,
...
<body>
<div class="topLanBar"><b>当前位置：</b>康复管理 >康复服务 > 新增感官功能康复记录</div>
<table width="100%">
  <tr>
    <td background="<%=basePath%>/pages/train/skins/img/lan-index.gif">&nbsp;</td>
  </tr> 
</table>
<form name="myform" method="post" action="sensorysaveSenchology.do">
...
<td colspan="6" align="right">所属项目:<input type="text" id="sbelongpro" name="sbelongpro" size="10" readonly onclick="choosePro()"></td>
</tr>
</table>
<table  width="100%" border="1" cellpadding="3" cellspacing="1" class="tableList">
    ...
    <tr>
     ...
     <td align="center"><input type="text" size="21" name="sstarttime" value="" class="Wdate" onClick="WdatePicker({dateFmt:'HH:mm:ss',readOnly:true})" /></td>
     <td align="center"><input type="text" size="21" name="sendtime" value="" class="Wdate" onClick="WdatePicker({dateFmt:'HH:mm:ss',readOnly:true})" /></td>
     <td align="center"><input type="text" name="sattend" value="${sattend}" size="28"/></td>
     <td align="center">
                       <textarea name="sinfo" cols="38" onBlur="javascript:this.style.width='202px';this.style.height='21px';" onFocus="javascript:this.style.width='210px';this.style.height='90px';" rows="1">${sinfo}</textarea>
     </td>
     <td align="center"><input type="text" name="swrite" value="${swrite}" size="14"/></td>
    </tr>
    </tbody>
</table>
<table width="100%">
