...
function onSubmit()
{
   ...
   for (var i = 1; i < tbody.rows.length; i++) {
            ...
            var item = pid + "^" + pnumber + "^" + pstarttime + "^" + pendtime + "^" + pattend + "^" + pinfo + "^" + pwrite + "^" + pbelongpro;
            detail.push(item);
        }
        //判断时间点之间的交错
        ...
        else
        {
	            $.ajax({
					type : "POST",
					url : basePath + "/psychologymodifypsy.do?detail=" + detailstr,
...
function psyDel(pnumber){
      ...
      if (ischeck) {
		  ...
		  if(but){
		       $.ajax({
			   type: "POST",
			   url: basePath + "/psychologydelchoosepsy.do", 
			   data: {pid:detailstr},
			   success:function (msg){
			   		var result = msg;
			   		if('success'==result){
...
<body>
<div class="topLanBar"><b>当前位置：</b>康复管理 >康复服务 > 修改心理康复</div>
<table width="100%">
  <tr>
...
<td colspan="6" align="right">所属项目:<input type="text" id="pbelongpro" name="pbelongpro" value="${result[0].pbelongpro}" size="10" onclick="choosePro()" readonly></td>
</tr>
</table>
<table width="100%" border="0" cellpadding="3" cellspacing="1" class="tableList" id="tbody">
	...
	<c:otherwise>
    <c:forEach var="psyc" items="${result}" varStatus="loopStatus">
    <tr>
          <input type="hidden" name="pid${loopStatus.count}" value="${psyc.pid}" />
          <input type="hidden" name="pnumber" value="${psyc.pnumber}" />
	      ...
	      <td align="center" nowrap bgcolor="#E4E4E4"><input type="text" size="21" name="pstarttime" value="${psyc.pstarttime}" class="Wdate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})" /></td>
	      <td align="center" nowrap bgcolor="#E4E4E4"><input type="text" size="21" name="pendtime" value="${psyc.pendtime}" class="Wdate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})" /></td>
	      <td align="center" nowrap bgcolor="#E4E4E4"><input name="pattend" type="text" value="${psyc.pattend}" size="28"/></td>
	      <td align="center" nowrap bgcolor="#E4E4E4"><textarea name="pinfo" cols="38" onBlur="javascript:this.style.width='202px';this.style.height='21px';" onFocus="javascript:this.style.width='210px';this.style.height='90px';" rows="1">${psyc.pinfo}</textarea></td>
	      <td align="center" nowrap bgcolor="#E4E4E4"><input name="pwrite" type="text" value="${psyc.pwrite}" size="14"/></td>
	</tr>
    </c:forEach>
    </c:otherwise>
    </c:choose>
