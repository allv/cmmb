...
function addFileInfo(fileId,message){
    ...
    var startindex=parseInt(fileId.substring(fileId.length-1))+previousRows;
    fileId=fileId.substring(0,fileId.length-1)+startindex;
...
function addReadyFileInfo(fileid,fileName,message,status){
	...
	var filename = document.getElementById("mimageurl");
	if (k == 0)
	{
	    mimageurl = filename.value;
	    k = 1;
	}
	imgurl = fileName + "^" + imgurl;
	filename.value = imgurl + mimageurl;
	var row = infoTable.insertRow();
	row.id = fileid.substring(0,fileid.length-1)+(infoTable.rows.length-1);
