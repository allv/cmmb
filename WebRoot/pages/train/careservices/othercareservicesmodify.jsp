...
function onSubmit()
{
   ...
   for (var i = 1; i < tbody.rows.length; i++) {
            var oid = document.getElementById("oid"+i).value;
            ...
            var item = oid + "^" + odate + "^" + oname + "^" + ocontent + "^" + ostarttime + "^" + oendtime + "^" + opeople + "^" + oevaluation + "^" + oinfo + "^" + obelongpro;
            detail.push(item);
        }
        ...
        else
        {
	            $.ajax({
					type : "POST",
					url : basePath + "/othercareservicesmodifycar.do?detail=" + encodeURI(detailstr),
...
function batDel(){
      ...
      if (ischeck) {
          for (var i = checkeds.length - 1; i >= 0; i--) {
              if (checkeds[i].checked) {
                 var num = i +1; 
                 var oid = document.getElementById("oid"+num).value;
                 var item = oid;
                 detail.push(item);
              }
          }
		  ...
		  if(but){
		       $.ajax({
			   type: "POST",
			   url: basePath + "/othercareservicesdelchoosecar.do", 
			   data: {oid:detailstr},
			   success:function (msg){
			   		var result = msg;
			   		if('success'==result){
...
<body>
<div class="topLanBar"><b>当前位置：</b>护理管理 >介护服务 > 修改介护服务记录</div>
<table width="100%">
  <tr>
...
<td colspan="9" align="right">所属项目:<input type="text" id="obelongpro" name="obelongpro" value="${result[0].obelongpro}" size="18" onclick="choosePro()" readonly></td>
</tr>
</table>
<table width="100%" border="0" cellpadding="3" cellspacing="1" class="tableList" id="tbody">
    ...
    <tr>
	      ...
	      <td align="center" nowrap bgcolor="#E4E4E4"><input name="oname" type="text" value="${car.oname}" size="22" onclick="chooseElders()" readonly/></td>
          <td align="center" nowrap bgcolor="#E4E4E4"><input name="ocontent" type="text" value="${car.ocontent}" size="19"/></td>
          <td align="center" nowrap bgcolor="#E4E4E4"><input type="text" size="21" name=ostarttime value="${car.ostarttime}" class="Wdate" onClick="WdatePicker({dateFmt:'HH:mm:ss',readOnly:true})" /></td>
          <td align="center" nowrap bgcolor="#E4E4E4"><input type="text" size="21" name="oendtime" value="${car.oendtime}" class="Wdate" onClick="WdatePicker({dateFmt:'HH:mm:ss',readOnly:true})" /></td>
          <td align="center" nowrap bgcolor="#E4E4E4"><input name="opeople" type="text" value="${car.opeople}" size="19"/></td>
          <td align="center" nowrap bgcolor="#E4E4E4"><input name="oevaluation" type="text" value="${car.oevaluation}" size="19"/></td>
          <td align="center" nowrap bgcolor="#E4E4E4"><textarea name="oinfo" cols="34" onBlur="javascript:this.style.width='186px';this.style.height='21px';" onFocus="javascript:this.style.width='210px';this.style.height='90px';" rows="1">${car.oinfo}</textarea></td>
	</tr>
    </c:forEach>
    </c:otherwise>
    </c:choose>
