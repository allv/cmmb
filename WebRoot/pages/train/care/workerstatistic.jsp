...
function onSubmit(){
   ...
   window.location.href=basePath+"/workerstatistic.do?workername="+memname+"&startDate="+startdate+"&endDate="+enddate+"&careflag=true";
}
function changeState(){
   ...
   $.ajax({
				   type: "POST",
				   url: basePath + "/carechangeStateForData.do?names="+encodeURI(ids)+"&startdate="+encodeURI(startdate)+"&enddate="+enddate+"&careflag=member1",
				   success:function (msg){
				   		var result = msg;
				   		if('success'==result){
				   		 	alert('数据提交成功');
				   			//window.location.href=basePath+"/workerstatistic.do?careflag=true";
				   		}
				   		else{
				   			alert('数据提交失败');
				   			return;
...
function cancelState(){
   ...
   $.ajax({
				   type: "POST",
				   url: basePath + "/carechangeStateForData.do?names="+encodeURI(ids)+"&startdate="+startdate+"&enddate="+enddate+"&careflag=member1"+"&isCancel=true",
				   success:function (msg){
				   		var result = msg;
				   		if('success'==result){
				   		 	alert('数据已取消提交');
